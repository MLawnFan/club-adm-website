...
interface ExerciseBlock {
  ...
  note?: string; // Block-level coaching note adapted to experience
  exercices: { nom: string; detail: string; videoUrl?: string; conseil?: string }[];
}
...
// ─── Program Generator ─────────────────────────────────────────
// ─── Experience-based coaching notes ──────────────────────────
function getBlockNote(blockTitle: string, experience: string, objectif: string): string | undefined {
  if (experience === "avance") {
    // Avancé: notes minimales, juste des cues d'intensité
    if (blockTitle === "Warm Up") return undefined;
    if (blockTitle === "Strength") return "RPE 8-9. Contrôle excentrique.";
    if (blockTitle === "Accessory Work") return "Superset si possible. Mind-muscle connection.";
    if (blockTitle === "Conditioning") return "Push à 90%+ effort.";
    if (blockTitle === "Cool Down") return undefined;
    return undefined;
  }
  if (experience === "intermediaire") {
    if (blockTitle === "Warm Up") return "Prends le temps de bien activer les muscles ciblés. 2-3 min de cardio léger + mouvements dynamiques.";
    if (blockTitle === "Strength") return "Focus sur la technique avant la charge. Augmente le poids seulement si la forme est parfaite.";
    if (blockTitle === "Accessory Work") return "Contrôle le mouvement — 2 sec en descendant, 1 sec en montant. Repos complets entre les séries.";
    if (blockTitle === "Conditioning") return "Maintiens une intensité élevée mais garde une forme propre. Réduis l'intensité si la technique se dégrade.";
    if (blockTitle === "Functional Circuit") return "Enchaîne les exercices avec un repos minimal entre chaque. Repos complet entre les rounds.";
    if (blockTitle === "Cool Down") return "Respire profondément pendant chaque étirement. Ne force jamais au-delà de l'inconfort léger.";
    return undefined;
  }
  // Débutant: explications détaillées
  if (blockTitle === "Warm Up") return "L'échauffement est crucial pour prévenir les blessures. Commence doucement et augmente progressivement l'intensité. Tu devrais sentir tes muscles se réchauffer et ton rythme cardiaque monter légèrement. Ne saute jamais cette étape.";
  if (blockTitle === "Strength") return "C'est le bloc principal. Commence TOUJOURS avec un poids que tu maîtrises parfaitement. La technique passe avant la charge. Si tu doutes de ta forme, réduis le poids. Repos complet entre chaque série — ne te presse pas. Respire : expire en poussant/tirant, inspire en contrôlant la descente.";
  if (blockTitle === "Accessory Work") return "Ces exercices complètent le travail principal. Utilise des poids plus légers et concentre-toi sur le muscle ciblé (mind-muscle connection). Descends lentement (2-3 secondes), monte de façon contrôlée. Si tu ne sens pas le muscle travailler, réduis le poids.";
  if (blockTitle === "Conditioning") return "Le but est d'élever ton cardio tout en gardant une bonne forme. Adapte l'intensité à ton niveau — il vaut mieux faire moins de reps proprement que beaucoup de reps avec une mauvaise technique. Prends des pauses si nécessaire.";
  if (blockTitle === "Functional Circuit") return "Enchaîne les exercices un après l'autre. Si tu as besoin de repos supplémentaire, prends-le. La qualité du mouvement est plus importante que la vitesse. Concentre-toi sur des mouvements fluides et contrôlés.";
  if (blockTitle === "Lower Body Strength") return "Concentre-toi sur la stabilité de tes genoux (ne les laisse pas rentrer vers l'intérieur). Garde le dos droit et le core engagé. Descends aussi bas que ta mobilité le permet sans douleur.";
  if (blockTitle === "HIIT Circuit") return "Haute intensité ne veut pas dire sacrifier la forme. Adapte le nombre de reps à ton niveau. Si tu es essoufflé au point de ne plus pouvoir maintenir la technique, ralentis ou prends une pause de 10-15 secondes.";
  if (blockTitle === "Cool Down") return "Le retour au calme aide à la récupération. Tiens chaque étirement au moins 30 secondes sans rebondir. Respire profondément et lentement. C'est le moment de relaxer ton système nerveux.";
  return "Prends ton temps et concentre-toi sur la qualité de chaque mouvement.";
}

function getExerciseConseil(exerciseName: string, experience: string): string | undefined {
  if (experience === "avance") return undefined; // Les avancés n'ont pas besoin de conseils par exercice
  
  const conseils: Record<string, { debutant: string; intermediaire: string }> = {
    // Gym - Upper
    "Développé couché (barre)": {
      debutant: "Pieds au sol, fesses et épaules collées au banc. Descends la barre au niveau des mamelons. Pousse en ligne droite. Demande un spot si tu es seul.",
      intermediaire: "Serre les omoplates, arque légèrement. Contrôle la descente 2-3 sec."
    },
    "Chest Press (machine)": {
      debutant: "Ajuste le siège pour que les poignées soient au niveau de ta poitrine. Pousse sans verrouiller les coudes. Contrôle le retour.",
      intermediaire: "Squeeze en haut du mouvement. Tempo 2-0-2."
    },
    "Développé militaire (haltères)": {
      debutant: "Assis avec le dos bien appuyé. Pars des épaules, pousse au-dessus de la tête sans cambrer le dos. Garde le core serré.",
      intermediaire: "Engage le core, évite de cambrer. Full ROM."
    },
    "Dips ou Push-ups déclinés": {
      debutant: "Si les dips sont trop difficiles, commence par des push-ups déclinés (pieds surélevés). Descends jusqu'à 90° aux coudes maximum.",
      intermediaire: "Penche-toi légèrement vers l'avant pour cibler les pecs."
    },
    "Tractions (ou Lat Pulldown)": {
      debutant: "Si tu ne peux pas faire de tractions, utilise le Lat Pulldown. Tire avec les coudes (pas les mains). Imagine que tu mets tes coudes dans tes poches arrière.",
      intermediaire: "Initie le mouvement par la dépression scapulaire. Descends jusqu'au menton au-dessus de la barre."
    },
    "Rowing barre": {
      debutant: "Dos à 45°, genoux légèrement fléchis. Tire la barre vers le nombril. Garde le dos DROIT — si tu arrondis, le poids est trop lourd.",
      intermediaire: "Squeeze les omoplates en haut. Contrôle la descente."
    },
    "Face Pulls": {
      debutant: "Poulie haute, corde. Tire vers ton visage en ouvrant les coudes vers l'extérieur. Excellent pour la posture et la santé des épaules.",
      intermediaire: "Rotation externe en fin de mouvement. Léger et contrôlé."
    },
    // Gym - Lower
    "Squat (barre)": {
      debutant: "Pieds largeur d'épaules, orteils légèrement vers l'extérieur. Descends comme si tu t'assoyais sur une chaise. Genoux dans l'axe des orteils. Garde la poitrine haute et le regard devant.",
      intermediaire: "Brise à la hanche d'abord. Vise la parallèle. Drive par les talons."
    },
    "Presse à cuisses": {
      debutant: "Pieds au milieu de la plateforme, largeur d'épaules. Ne verrouille JAMAIS les genoux en haut. Descends jusqu'à 90° aux genoux.",
      intermediaire: "Pieds hauts = plus de fessiers. Pieds bas = plus de quads."
    },
    "Fentes marchées": {
      debutant: "Grands pas, genou arrière frôle le sol. Garde le torse droit. Si tu perds l'équilibre, fais des fentes statiques d'abord.",
      intermediaire: "Pousse par le talon avant. Contrôle la descente."
    },
    "Soulevé de terre roumain": {
      debutant: "Dos DROIT obligatoire. Pousse les fesses vers l'arrière, barre le long des jambes. Sens l'étirement dans les ischio-jambiers. Arrête quand tu sens que ton dos veut arrondir.",
      intermediaire: "Hinge à la hanche, micro-flexion genoux. Sens l'étirement ischio à mi-tibia."
    },
    "Hip Thrust (barre)": {
      debutant: "Dos appuyé sur un banc au niveau des omoplates. Pousse les hanches vers le plafond en serrant les fessiers. Menton vers la poitrine en haut.",
      intermediaire: "Pause 1-2 sec en haut. Squeeze maximal des fessiers."
    },
    // Accessory
    "Extensions triceps (poulie)": {
      debutant: "Coudes collés au corps, ne bouge que les avant-bras. Squeeze en bas du mouvement.",
      intermediaire: "Coudes fixes. Contrôle la phase excentrique."
    },
    "Élévations latérales": {
      debutant: "Léger! Monte les bras sur les côtés jusqu'à la hauteur des épaules. Coudes légèrement fléchis. Ne balance pas le corps.",
      intermediaire: "Initie par les deltoïdes, pas par les trapèzes. Léger et contrôlé."
    },
    "Curl biceps (haltères)": {
      debutant: "Coudes collés au corps, ne balance pas. Monte en tournant le poignet (supination). Descends lentement.",
      intermediaire: "Pas de momentum. Squeeze en haut 1 sec."
    },
    // Maison
    "Pompes (variante adaptée)": {
      debutant: "Si les pompes au sol sont trop difficiles, commence sur les genoux ou contre un mur. Corps en planche, descends la poitrine vers le sol. Coudes à 45°, pas sur les côtés.",
      intermediaire: "Protraction scapulaire en haut. Tempo 2-1-1."
    },
    "Squats (poids du corps ou lestés)": {
      debutant: "Pieds largeur d'épaules. Descends comme si tu t'assoyais. Garde les talons au sol et la poitrine haute. Genoux dans l'axe des orteils.",
      intermediaire: "Ajoute du poids (sac à dos, gallon d'eau) pour progresser."
    },
    "Burpees": {
      debutant: "Décompose le mouvement : squat → mains au sol → recule les pieds → pompe (optionnelle) → ramène les pieds → saute. Vas-y à ton rythme, la forme avant la vitesse.",
      intermediaire: "Mouvement fluide et explosif. Poitrine touche le sol."
    },
    "Kettlebell Swings": {
      debutant: "C'est un mouvement de HANCHE, pas de bras. Pousse les fesses en arrière, puis propulse les hanches vers l'avant. Le kettlebell monte grâce à la force des hanches. Garde les bras détendus.",
      intermediaire: "Hinge explosif. Squeeze les fessiers en haut. Bras = pendule."
    },
    "Planche": {
      debutant: "Sur les avant-bras, corps en ligne droite de la tête aux talons. Serre les abdos et les fessiers. Ne laisse pas les hanches descendre OU monter.",
      intermediaire: "Engage le core comme si on allait te frapper au ventre. Respire normalement."
    },
  };
  
  const c = conseils[exerciseName];
  if (!c) {
    // Conseil générique selon le niveau
    if (experience === "debutant") return "Concentre-toi sur la forme. Si tu n'es pas sûr du mouvement, regarde la vidéo avant de commencer.";
    return undefined;
  }
  return experience === "debutant" ? c.debutant : c.intermediaire;
}
...
function generateProgram(profile: TrainingProfile): TrainingPlan {
  ...
  for (let w = 1; w <= 4; w++) {
    ...
    for (let d = 0; d < profile.joursParSemaine; d++) {
      ...
      // Add experience-based notes to each block and exercise
      day.blocs = day.blocs.map(bloc => ({
        ...bloc,
        note: getBlockNote(bloc.titre, profile.experience, profile.objectif),
        exercices: bloc.exercices.map(ex => ({
          ...ex,
          conseil: getExerciseConseil(ex.nom, profile.experience),
        })),
      }));
                ...
                {currentDay.blocs.map((bloc, bi) => (
                  <div key={bi} className="bg-card border border-border rounded-xl overflow-hidden">
                    ...
                    <div className="px-4 py-3 border-b border-border bg-secondary/20">
                      ...
                      {bloc.note && (
                        <p className="text-xs text-blue-400/90 mt-2 leading-relaxed italic">
                          💡 {bloc.note}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="p-4 space-y-2">
                      {bloc.exercices.map((ex, ei) => (
                        <div key={ei} className="py-1.5">
                          ...
                          {/* Per-exercise coaching tip */}
                          {ex.conseil && (
                            <div className="mt-1 ml-0 pl-3 border-l-2 border-blue-500/30">
                              <p className="text-xs text-blue-300/80 leading-relaxed">{ex.conseil}</p>
                            </div>
                          )}
                        </div>
                      ))}
