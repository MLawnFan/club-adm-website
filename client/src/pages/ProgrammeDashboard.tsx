...
import { Lock, Trophy, Zap, Flame, Star, ChevronRight, ArrowLeft } from "lucide-react";
  ...
  if (!authLoading && !isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: "#0f1229" }}>
        ...
        >
          <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6" style={{ backgroundColor: "rgba(237,28,36,0.12)" }}>
            <Lock size={36} style={{ color: "#ed1c24" }} />
          </div>
          <h1 className="text-3xl lg:text-4xl text-white mb-4" style={{ fontFamily: "var(--font-display)" }}>
            ESPACE MEMBRE
          </h1>
          <p className="text-base mb-8" style={{ color: "rgba(255,255,255,0.5)", fontFamily: "var(--font-body)" }}>
            Connecte-toi pour accéder à ton programme On R'start la Machine et suivre ta progression.
          </p>
          <a
            href={getLoginUrl()}
            className="inline-flex items-center gap-2 px-8 py-3.5 text-white text-sm font-bold uppercase tracking-wider rounded-lg transition-all duration-300 hover:shadow-lg hover:shadow-red-500/20 hover:scale-105"
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: "#0f1229" }}>
        ...
        >
          ...
          <h1 className="text-4xl lg:text-5xl text-white mb-4" style={{ fontFamily: "var(--font-display)" }}>
            ON R'START<br /><span style={{ color: "#ed1c24" }}>LA MACHINE</span>
          </h1>
          <p className="text-base mb-4 text-white/50" style={{ fontFamily: "var(--font-body)" }}>
            Programme de transformation en 12 semaines
          </p>
          <p className="text-sm mb-8 text-white/35 max-w-sm mx-auto" style={{ fontFamily: "var(--font-body)" }}>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-8 py-4 text-white text-sm font-bold uppercase tracking-wider rounded-lg transition-all duration-300 hover:shadow-lg hover:shadow-red-500/20 hover:scale-105 disabled:opacity-50"
            style={{ backgroundColor: "#ed1c24", fontFamily: "var(--font-body)" }}
          >
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#0f1229" }}>
        ...
        <div className="max-w-[1280px] mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-white/40">Salut, {user?.name || "Membre"} 👋</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full" style={{ backgroundColor: "rgba(237,28,36,0.12)" }}>
              <Zap size={14} style={{ color: "#ed1c24" }} />
              <span className="text-sm font-bold text-white">{enrollment.totalXp} XP</span>
            </div>
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full" style={{ backgroundColor: `${currentLevel.color}20` }}>
              <Star size={14} style={{ color: currentLevel.color }} />
              <span className="text-sm font-bold" style={{ color: currentLevel.color }}>{currentLevel.name}</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-[1280px] mx-auto px-6 py-8">
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="mb-8 p-6 rounded-xl border border-white/[0.06]"
        ...
        >
          ...
          <div className="flex items-center justify-between mt-3">
            <span className="text-xs text-white/30">{completedModules.length} / 14 modules</span>
            <div className="flex items-center gap-1">
              <Flame size={12} style={{ color: "#ed1c24" }} />
              <span className="text-xs text-white/50">Streak: {enrollment.streak}</span>
            </div>
          </div>
        </motion.div>
        ...
        {earnedBadges.length > 0 && (
          <motion.div
            ...
            className="mb-8"
          >
            <h3 className="text-sm font-bold uppercase tracking-wider text-white/50 mb-4">Tes badges</h3>
            <div className="flex flex-wrap gap-3">
              {earnedBadges.map(badge => (
                <div
                  key={badge.id}
                  className="flex items-center gap-2 px-3 py-2 rounded-lg border border-white/[0.08]"
                  style={{ backgroundColor: "rgba(237,28,36,0.06)" }}
                >
        ...
        {/* Modules grid */}
        <h3 className="text-sm font-bold uppercase tracking-wider text-white/50 mb-4">Les modules</h3>
            ...
            return (
              ...
              >
                {isUnlocked ? (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold uppercase tracking-wider" style={{ color: isCompleted ? "#22c55e" : "#ed1c24" }}>
                            {module.title}
                          </span>
                          {isCurrent && (
                            <span className="text-[10px] px-2 py-0.5 rounded-full font-bold text-white" style={{ backgroundColor: "#ed1c24" }}>
                              EN COURS
                            </span>
                          )}
                        </div>
                  ...
                  <div
                    className="flex items-center gap-4 p-4 rounded-xl border border-white/[0.04] opacity-50"
                    style={{ backgroundColor: "rgba(255,255,255,0.01)" }}
                  >
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: "rgba(255,255,255,0.05)" }}>
                      <Lock size={16} className="text-white/30" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <span className="text-xs font-bold uppercase tracking-wider text-white/30">{module.title}</span>
                      <p className="text-sm text-white/40 font-medium mt-0.5">{module.subtitle}</p>
                      <p className="text-xs text-white/20 mt-0.5">Complète le module précédent pour débloquer</p>
                    </div>
