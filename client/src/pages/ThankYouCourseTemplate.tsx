/*
 * COMPOSANT RÉUTILISABLE — Thank You Course
 * Utilisé par les 4 pages de remerciement (une par distance)
 * Chaque page passe sa distance et son webhook URL en props
 */
...
interface ThankYouCourseProps {
  distance: string; // ex: "21.1 km", "10 km", "5 km", "1 km"
      ...
      <section className="py-20 lg:py-32 relative overflow-hidden">
        ...
        <div className="max-w-[800px] mx-auto px-6 lg:px-8 text-center relative z-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.6 }}>
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-6" style={{ backgroundColor: "rgba(237,28,36,0.12)", border: "1px solid rgba(237,28,36,0.25)" }}>
              <span className="text-xs font-bold uppercase tracking-wider" style={{ color: "#ed1c24", fontFamily: "var(--font-body)" }}>{distanceLabel}</span>
            </div>
            ...
            <p className="text-lg lg:text-xl leading-relaxed mb-4" style={{ color: "rgba(255,255,255,0.7)", fontFamily: "var(--font-body)" }}>
              Ton inscription au <strong>{distanceLabel}</strong> du 19 septembre est confirmée.
            </p>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-xl p-8 lg:p-10 border" style={{ backgroundColor: "rgba(237,28,36,0.04)", borderColor: "rgba(237,28,36,0.3)" }}>
            {isSubmitted ? (
              <div className="text-center py-4">
                ...
                <p className="text-sm" style={{ color: "rgba(255,255,255,0.6)", fontFamily: "var(--font-body)" }}>Ton plan de course ({distance}) est en route vers ta boîte courriel. Vérifie tes courriels indésirables si tu ne le vois pas dans les prochaines minutes.</p>
              </div>
            ) : (
              <>
                <div className="text-center mb-6">
                  ...
                  <p className="text-sm" style={{ color: "rgba(255,255,255,0.5)", fontFamily: "var(--font-body)" }}>Remplis les infos suivantes pour recevoir ta programmation <strong>{distance}</strong> personnalisée directement par courriel.</p>
                </div>
                <form onSubmit={handleSubmitPlan} className="space-y-4">
                  ...
                  >
                    {isSubmitting ? "Envoi en cours..." : `Recevoir mon plan ${distance}`}
                  </button>
                </form>
              </>
            )}
        ...
        <div className="max-w-[800px] mx-auto px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-xl p-8 lg:p-10 border border-white/[0.06]" style={{ backgroundColor: "rgba(255,255,255,0.03)" }}>
            ...
            <p className="text-base leading-relaxed mb-6" style={{ color: "rgba(255,255,255,0.5)", fontFamily: "var(--font-body)" }}>
              En tant que participant inscrit au {distanceLabel}, tu reçois une programmation de course sur 3 entraînements par semaine pour te préparer au jour J :
            </p>
